...
function BulkScanModal({
  ...
  function toggleBarcode(raw: string) {
    ...
    setBarcodes((prev) => (prev.includes(value) ? prev.filter((b) => b !== value) : [value, ...prev]));
  }
    ...
    <Modal title="Scan barcodes" label="Bulk scan" onClose={onClose} wide>
      <form onSubmit={submit} className="space-y-5">
        <div className="rounded-xl border border-black bg-white px-4 py-3 text-sm leading-6 text-black">
          Scan many barcodes, then choose one name. Scanning the same code again removes it from this batch. Saving an
          already-cataloged code under the same name removes it from the master catalog.
        </div>
