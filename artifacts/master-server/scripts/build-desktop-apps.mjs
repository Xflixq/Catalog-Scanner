...
async function packageApp({ productName, entryRelative, outName }) {
  ...
  const outputPaths = await packager({
    ...
    appUserModelId: outName === 'DTMInventoryMaster' ? 'com.dtm.inventory.master' : 'com.dtm.inventory.setup',
    win32metadata: {
      CompanyName: 'DTM',
...
// Prefer MSI for public downloads. Attempt WiX MSI build on Windows.
function tryBuildMsi() {
  if (process.platform !== 'win32') {
    console.log('MSI build skipped (not Windows). Run installer/build-msi-apps.ps1 on Windows.');
    return;
  }
  const ps1 = path.join(pkgRoot, 'installer', 'build-msi-apps.ps1');
  if (!exists(ps1)) return;
  ...
  if (r.status !== 0) {
    console.warn('MSI build failed or WiX missing. Install WiX: dotnet tool install --global wix');
  }
}
tryBuildMsi();

// Ensure downloads folder has MSI if produced; keep exe as fallback only (not listed preferred)
for (const name of ['DTMInventoryMaster.msi', 'DTMInventorySetup.msi']) {
  const from = path.join(installerDir, name);
  if (exists(from)) fs.copyFileSync(from, path.join(downloadsDir, name));
}
// Fallback exe only if MSI missing (download page prefers MSI names)
if (!exists(path.join(downloadsDir, 'DTMInventorySetup.msi')) && setupExe) {
  fs.copyFileSync(setupExe, path.join(downloadsDir, 'DTMInventorySetup.exe'));
}
if (!exists(path.join(downloadsDir, 'DTMInventoryMaster.msi')) && masterExe) {
  fs.copyFileSync(masterExe, path.join(downloadsDir, 'DTMInventoryMaster.exe'));
}
