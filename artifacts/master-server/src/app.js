...
export function createApp(config) {
  ...
  function listDownloadFiles(dir) {
    const wanted = [
      { key: 'windowsSetup', names: ['DTMInventory-Setup-Windows.zip', 'DTMInventorySetup-App-Win64.zip', 'DTMInventorySetup.exe'], label: 'Windows Setup (non-Master)' },
      { key: 'windowsMaster', names: ['DTMInventory-Master-Windows.zip', 'DTMInventoryMaster-App-Win64.zip', 'DTMInventoryMaster.exe'], label: 'Windows Master' },
  ...
  app.get('/', async (req, res) => {
    ...
    const winCard = windows
      ? `<a class="card action" href="${windows.url}">
          ...
          <p>Non-master Windows install package for warehouse PCs.</p>
...
<body>
  <div class="wrap">
    ...
    <footer>
      ...
      <div>
        Pairing API is on this same address. Open the Master desktop app to manage inventory.
