...
export function listGroupedCatalog(db) {
  ...
  const items = db
    .prepare(
      `SELECT id, barcode, name_id as nameId, notes,
              created_at as createdAt, updated_at as updatedAt,
              COALESCE(NULLIF(scanned_at, ''), created_at) as scannedAt
       FROM items
 ...
 * @param {import('better-sqlite3').Database} db
 * @param {{ name: string, barcodes: string[], notes?: string, scannedAt?: string }} payload
 */
export function applyBulkScan(db, payload) {
  ...
  let moved = 0;
  const now = nowIso();
  const scannedAt = payload.scannedAt && !Number.isNaN(Date.parse(payload.scannedAt))
  ...
  const insertItem = db.prepare(
    `INSERT INTO items (id, barcode, name_id, notes, created_at, updated_at, scanned_at)
     VALUES (@id, @barcode, @nameId, @notes, @createdAt, @updatedAt, @scannedAt)`,
  );
  const deleteItem = db.prepare(`DELETE FROM items WHERE id = ?`);
  const updateItem = db.prepare(
    `UPDATE items
     SET name_id = @nameId, notes = @notes, updated_at = @updatedAt, scanned_at = @scannedAt
     WHERE id = @id`,
  );
