...
// Windows taskbar / jump-list branding (stops "Electron" label)
const APP_ID = 'com.dtm.inventory.master';
try {
  app.setName('DTM Inventory Master');
...
function wireIpc() {
  ...
  ipcMain.handle('master:pins', async () => {
    return requestJson('GET', '/api/master/pins');
  });
  ipcMain.handle('master:pinToggle', async (_e, payload) => {
    return requestJson('POST', '/api/master/pins/toggle', payload || {});
  });
