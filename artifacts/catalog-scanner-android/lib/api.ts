...
export async function saveSession(session: SessionInfo) {
...
export function parseTetherPayload(raw: string): { baseUrl: string; lanIps?: string[] } {
  const text = raw.trim();
  // Accept raw JSON QR or plain URL.
  if (text.startsWith('{')) {
    const parsed = JSON.parse(text) as { baseUrl?: string; lanIps?: string[]; type?: string };
    if (!parsed.baseUrl) throw new Error('Tether QR is missing baseUrl');
    return { baseUrl: normalizeBaseUrl(parsed.baseUrl), lanIps: parsed.lanIps };
  }
  if (/^https?:\/\//i.test(text)) {
    return { baseUrl: normalizeBaseUrl(text) };
  }
  throw new Error('Unrecognized tether QR');
}

export async function connectWithTether(qrText: string, deviceName: string) {
  const tether = parseTetherPayload(qrText);
  const candidates = [tether.baseUrl, ...(tether.lanIps || []).map(normalizeBaseUrl)];
  let lastError: Error | null = null;

  for (const baseUrl of candidates) {
    try {
      await request(null, '/api/health', {}, baseUrl);
      const session = await request<SessionInfo>(
        null,
        '/api/auth/tether',
        {
          method: 'POST',
          body: JSON.stringify({ baseUrl, deviceName }),
        },
        baseUrl,
      );
      const full = { ...session, baseUrl };
      await saveSession(full);
      return full;
    } catch (error) {
      lastError = error instanceof Error ? error : new Error('Tether failed');
    }
  }
  throw lastError || new Error('Could not reach master PC');
}
...
export async function bulkScan(
  session: SessionInfo,
  payload: { name: string; barcodes: string[]; notes?: string },
) {
  return request<{
    added: number;
    removed: number;
