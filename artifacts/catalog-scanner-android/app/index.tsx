...
function BulkScanModal({
  ...
  onSubmit: (payload: { name: string; barcodes: string[]; notes: string }) => Promise<void>;
}) {
  const colors = useColors();
  const styles = createStyles(colors);
  const [permission, requestPermission] = useCameraPermissions();
  ...
  function toggleBarcode(raw: string) {
  ...
  async function submit() {
    ...
    setError('');
    try {
      await onSubmit({ name: selectedName.trim(), barcodes, notes: notes.trim() });
      onClose();
    } catch (err) {
  ...
  return (
    <KeyboardAwareScrollViewCompat contentContainerStyle={styles.formContent} keyboardShouldPersistTaps="handled">
      <View style={styles.formIntro}>
        <Text style={styles.formIntroTitle}>Scan many barcodes, then choose one name for the batch.</Text>
        <Text style={styles.formIntroText}>Scanning the same code again removes it from this batch. On the master catalog, a second submit of an existing code removes it.</Text>
      </View>
      ...
      {Platform.OS !== 'web' && permission?.granted ? (
        <View style={styles.cameraFrame}>
          <CameraView
            ...
            onBarcodeScanned={({ data }) => toggleBarcode(data)}
          />
          <View style={styles.scanTarget} />
          <Text style={styles.cameraHint}>{barcodes.length} in batch</Text>
        </View>
      ) : Platform.OS !== 'web' ? (
        <Pressable onPress={() => void requestPermission()} style={styles.primaryButton}>
          <Text style={styles.primaryButtonText}>Allow camera</Text>
      ...
      <View style={styles.manualRow}>
        <TextInput
          ...
          placeholderTextColor={colors.mutedForeground}
        />
        <Pressable onPress={() => { toggleBarcode(manual); setManual(''); }} style={styles.secondaryButton}>
      ...
      <View style={styles.batchList}>
        {barcodes.length ? barcodes.map((code) => (
          <Pressable key={code} onPress={() => toggleBarcode(code)} style={styles.batchChip}>
